...
export const createCustomer = async (req, res) => {
  ...
  try {
    ...
    if (error.name === "ZodError") {
      return res
        .status(400)
        .json({ message: "Dados inválidos", details: error.flatten() });
    }
    ...
    if (error.code === "23505") {
      return res
        .status(409)
        .json({ message: "Este documento (CPF/CNPJ) já foi cadastrado." });
    }
...
export const updateCustomer = async (req, res) => {
  ...
  try {
    ...
    if (fields.length === 0) {
      return res
        .status(400)
        .json({ message: "Nenhum campo válido para atualização fornecido." });
    }
    ...
    if (error.name === "ZodError") {
      return res.status(400).json({
        message: "Dados inválidos na atualização",
    ...
    if (error.code === "23505") {
      return res
        .status(409)
        .json({ message: "Outro cliente já possui este CPF/CNPJ." });
    }
  ...
  try {
    ...
    if (error.code === "23503") {
      return res.status(409).json({
        message:
          "Não é possível excluir: o cliente possui dados vinculados (ex: pedidos).",
      });
    }
    res.status(500).json({ message: "Erro interno no servidor" });
  }
